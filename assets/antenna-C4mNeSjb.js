import"./modulepreload-polyfill-B5Qt9EMX.js";import{G as j,f as J,b as V,p as Q,l as tt}from"./fetch-B8wCyjRJ.js";const et="sat-groundtrack-state",D=j[0];let I=[],P=null,E=0,u=[],x=0,N=!1,z=null,U=1;function K(){at(),st(),nt()}function at(){try{const e=localStorage.getItem(et);if(!e)return;const a=JSON.parse(e);Array.isArray(a.satellites)&&(I=a.satellites.map(t=>({noradId:t.noradId,name:t.name,color:t.color,satrec:null,passes:null})),P=I.length>0?I[0].noradId:null)}catch{}}async function nt(){const e=document.getElementById("ant-status");for(const a of I)try{e&&(e.textContent=`${a.name} TLE yükleniyor...`);const t=await J(a.noradId);a.satrec=V(t.line1,t.line2),a.name=t.name,a.passes=Q(a.satrec,D,7)}catch{e&&(e.textContent=`${a.name} TLE yüklenemedi`)}e&&(e.textContent=""),X()}function st(){const e=document.getElementById("antenna-app");e.innerHTML=`
    <header class="ant-header">
      ...
      <div class="ant-gs">${D.name} — ${D.lat.toFixed(4)}°N, ${D.lon.toFixed(4)}°E, ${D.alt}m</div>
